...
Class Patient extends CI_Model
{
	...
	function addPatientInformationVersion($id, $name, $date, $status, $approver){
		...
		$query1 = $this->db->get('patientinfoversion');
		$res1 = $query1->row_array();

		if($query1 -> num_rows() >= 1) $maxpatientinfoverID = $res1['patientinfoversionID'];

		$data = array(
			'patientinfoversionID' => $maxpatientinfoverID+1,
			'UPCD_ID' => $id,
			'updatedBy' => $name,
			'updateDate' => $date,
			'updateStatus' => $status,
	...
	function addPatientChecklistVersion($id, $name, $date, $status, $approver){
		...
		$query1 = $this->db->get('patientchecklistversion');
		$res1 = $query1->row_array();

		if($query1 -> num_rows() >= 1) $maxchecklistverID = $res1['patientchecklistversionID'];
		$data = array(
			'patientchecklistversionID' => $maxchecklistver+1,
			'UPCD_ID' => $id,
			'updatedBy' => $name,
			'updateDate' => $date,
			'updateStatus' => $status,
	...
	function addMedAndSocHistoVersion($id, $name, $date, $status, $approver){
		...
		$query1 = $this->db->get('medandsochistoversion');
		$res1 = $query1->row_array();

		if($query1 -> num_rows() >= 1) $maxmedandsochistoverID = $res1['medandsochistoversionID'];
		$data = array(
			'medandsochistoversionID' => $maxmedandsochistoverID+1,
			'UPCD_ID' => $id,
			'updatedBy' => $name,
			'updateDate' => $date,
			'updateStatus' => $status,
	...
	function addDentalDataVersion($id, $name, $date, $status, $approver){
		...
		$query1 = $this->db->get('dentaldataversion');
		$res1 = $query1->row_array();

		if($query1 -> num_rows() >= 1) $maxdentaldataverID = $res1['dentaldataversionID'];

		$data = array(
			'dentaldataversionID' => $maxdentaldataverID+1,
			'UPCD_ID' => $id,
			'updatedBy' => $name,
			'updateDate' => $date,
			'updateStatus' => $status,
	...
	function addDentalChartVersion($id, $name, $date, $status, $approver){
		...
		$query1 = $this->db->get('dentalchartversion');
		$res1 = $query1->row_array();

		if($query1 -> num_rows() >= 1) $maxdentalchartverID = $res1['dentalchartversionID'];

		$data = array(
			'dentalchartversionID' => $maxdentalchartverID+1,
			'UPCD_ID' => $id,
			'updatedBy' => $name,
			'updateDate' => $date,
			'updateStatus' => $status,
	...
	function addTreatmentPlanVersion($id, $name, $date, $status, $approver){
		...
		$query1 = $this->db->get('treatmentplanversion');
		$res1 = $query1->row_array();

		if($query1 -> num_rows() >= 1) $maxtreatmentplanverID = $res1['treatmentplanversionID'];

		$data = array(
			'treatmentplanversionID' => $treatmentplanverID+1,
			'UPCD_ID' => $id,
			'updatedBy' => $name,
			'updateDate' => $date,
			'updateStatus' => $status,
	...
	function addRadiographicExamVersion($id, $name, $date, $status, $approver){
		...
		$query1 = $this->db->get('radioexamversion');
		$res1 = $query1->row_array();

		if($query1 -> num_rows() >= 1) $maxradioexamverID = $res1['radioexamversionID'];

		$data = array(
			'radioexamversionID' => $maxradioexamverID+1,
			'UPCD_ID' => $id,
			'updatedBy' => $name,
			'updateDate' => $date,
			'updateStatus' => $status,
	...
	function addServicesRenderedVersion($id, $name, $date, $status, $approver){
		...
		$query1 = $this->db->get('servicesrenderedversion');
		$res1 = $query1->row_array();

		if($query1 -> num_rows() >= 1) $maxservicesrenderedverID = $res1['servicesrenderedversionID'];
		$data = array(
			'servicesrenderedversionID' => $maxservicesrenderedverID+1,
			'UPCD_ID' => $id,
			'updatedBy' => $name,
			'updateDate' => $date,
			'updateStatus' => $status,
	...
	function addConFindVersion($id, $name, $date, $status, $approver){
		...
		$query1 = $this->db->get('confindversion');
		$res1 = $query1->row_array();

		if($query1 -> num_rows() >= 1) $maxconfindverID = $res1['confindversionID'];

		$data = array(
			'confindversionID' => $maxconfindverID+1,
			'UPCD_ID' => $id,
			'updatedBy' => $name,
			'updateDate' => $date,
			'updateStatus' => $status,
...
//		$query = $this -> db -> get();
		...
		if($demo == "or" || $demo == "and"){
			...
			if($dentdemo == "" && $servdemo == ""){
				...
				$txt = $txt." AND ($gender AND patient.age >= $agefrom AND patient.age <= $ageto AND patient.city='$city' $occs $demo (";
...
//WHERE patient.UPCD_ID = serviceneeded.UPCD_ID AND patient.UPCD_ID = patientinfo.UPCD_ID AND patient.UPCD_ID = treatmentplan.UPCD_ID AND patient.UPCD_ID = dentalchart.UPCD_ID AND patient.UPCD_ID = cariesstatus.UPCD_ID AND patient.UPCD_ID = recurrentstatus.UPCD_ID AND patient.UPCD_ID = restorationstatus.UPCD_ID AND WHERE patient.UPCD_ID = patientinfo.UPCD_ID AND $gender AND patient.age >= $agefrom AND patient.age <= $ageto AND patient.city LIKE '%$city%' $occs $demo ( ($perios OR $rpds OR $orthos OR $oss OR $fpds OR $pedos OR $endos OR $cds OR $restos) $demo ($carvar OR $recvar OR $resvar OR $remvar OR $extvar OR $intvar OR $mdrvar OR $ddrvar OR $rotvar OR $pccvar OR $rctvar OR $pafvar OR $exrvar OR $misvar OR $unevar OR $impvar OR $porvar OR $acrvar OR $metvar OR $pftvar) $demo ($c1var OR $c2var OR $c3var OR $c4var OR $c5var OR $olvar OR $exvar OR $odvar OR $scvar OR $pdvar OR $orvar OR $psvar OR $rcvar OR $tfvar OR $lmvar OR $anvar OR $povar OR $bsvar OR $sivar))"
			...
			if($patinfo != ""){
				$txt = "SELECT * FROM  patient, $patinfo WHERE patient.UPCD_ID = patientinfo.UPCD_ID AND $gender AND patient.age >= $agefrom AND patient.age <= $ageto AND patient.city='$city' $occs";
				//echo "elseif5 = $txt";

				$query = $this->db->query($txt);
			}
			else{ 
			$txt = "SELECT * FROM  patient WHERE $gender AND patient.age >= $agefrom AND patient.age <= $ageto AND patient.city='$city'";
			//echo "elseif6 = $txt";
			$query = $this->db->query($txt);
			}
		}
...
//		$query = $this -> db -> get();
		...
		if($demo == "or" || $demo == "and"){
			...
			if($dentdemo == "" && $servdemo == ""){
				...
				$txt = $txt." AND ($gender AND patient.age >= $agefrom AND patient.age <= $ageto AND patient.city='$city'  $demo (";
...
//WHERE patient.UPCD_ID = serviceneeded.UPCD_ID AND patient.UPCD_ID = patientinfo.UPCD_ID AND patient.UPCD_ID = treatmentplan.UPCD_ID AND patient.UPCD_ID = dentalchart.UPCD_ID AND patient.UPCD_ID = cariesstatus.UPCD_ID AND patient.UPCD_ID = recurrentstatus.UPCD_ID AND patient.UPCD_ID = restorationstatus.UPCD_ID AND WHERE patient.UPCD_ID = patientinfo.UPCD_ID AND $gender AND patient.age >= $agefrom AND patient.age <= $ageto AND patient.city LIKE '%$city%' $occs $demo ( ($perios OR $rpds OR $orthos OR $oss OR $fpds OR $pedos OR $endos OR $cds OR $restos) $demo ($carvar OR $recvar OR $resvar OR $remvar OR $extvar OR $intvar OR $mdrvar OR $ddrvar OR $rotvar OR $pccvar OR $rctvar OR $pafvar OR $exrvar OR $misvar OR $unevar OR $impvar OR $porvar OR $acrvar OR $metvar OR $pftvar) $demo ($c1var OR $c2var OR $c3var OR $c4var OR $c5var OR $olvar OR $exvar OR $odvar OR $scvar OR $pdvar OR $orvar OR $psvar OR $rcvar OR $tfvar OR $lmvar OR $anvar OR $povar OR $bsvar OR $sivar))"
			...
			if($patinfo != ""){
				$txt = "SELECT * FROM  patient, $patinfo WHERE patient.UPCD_ID = patientinfo.UPCD_ID AND $gender AND patient.age >= $agefrom AND patient.age <= $ageto AND patient.city='$city' ";
				//echo "elseif5 = $txt <br>";
