	...
	<table frame="box" class="frame alttable" style="width:98%; left:1%;">
		...
		if($agexfrom != '' && $agexto != ''){
			...
			echo "<tr>
				<td><b> AGE GROUP ($agefrom - $ageto) </b>
					...
					foreach($patientmatch as $row){
						if($row['gender'] == "Female" && $row['age'] >= $agefrom && $row['age'] <= $ageto){
							$femaleage++;
							$totalage++;
						}
						if($row['gender'] == "Male" && $row['age'] >= $agefrom && $row['age'] <= $ageto){
							$maleage++;
							$totalage++;
						}
					}
			...
			echo "<tr> 
				<td> Number of Females
				<td> $femaleage
				<td> ".round(($femaleage/$totalage)*100)."%
			</tr>
			<tr> 
				<td> Number of Males
				<td> $maleage
				<td> ".round(($maleage/$totalage)*100)."%
			</tr>";
		}
		?>
	</table><br>
