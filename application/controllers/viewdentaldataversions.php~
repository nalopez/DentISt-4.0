...
class ViewPatientInfoVersions extends CI_Controller {
		...
		if($this->session->userdata('logged_in'))
   		{
			...
			if($bool){
				...
				$id = $session_data['id'];
	   			...
	   			$data2['version'] = $this->patient->getPatientInfoVersions($id);
				...
				$this->load->view('viewpatientinfoversions_view', $data2);
			}
			else{
